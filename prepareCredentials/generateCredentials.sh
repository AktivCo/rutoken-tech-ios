...
# Generate CSR for the CA
$openssl_path \
  req -engine rtengine -utf8 -new -md_gost12_256 \
  -utf8 -config "$script_dir/ca.conf" -key "$credentials_dir/ca.key" -out "$credentials_dir/ca.csr"

# Sign CSR of the CA with Root CA
$openssl_path \
  x509 -extfile "$script_dir/ca.conf" -extensions ext -engine rtengine -req -md_gost12_256 -CA "$credentials_dir/rootCa.pem" -CAkey "$credentials_dir/rootCa.key" \
  -in "$credentials_dir/ca.csr" -out "$credentials_dir/ca.pem" -days 3650 -CAcreateserial
...
# Sign CSR of the Bank with the CA
$openssl_path \
  x509 -extfile "$script_dir/bank.conf" -extensions ext -engine rtengine -req -md_gost12_256 -CA "$credentials_dir/ca.pem" -CAkey "$credentials_dir/ca.key" \
